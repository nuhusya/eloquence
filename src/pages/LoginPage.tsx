...
export default function LoginPage() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    // Sanitize username: remove all spaces for the email part
    const sanitizedUsername = identifier.trim().replace(/\s+/g, '').toLowerCase();
    const virtualEmail = identifier.includes('@') ? identifier.trim() : `${sanitizedUsername}@eloquence.app`;
    ...
    try {
      if (mode === 'signup') {
        ...
        // If email confirmation is required, the user won't have a session
        const isNotConfirmed = signUpData.user && !signUpData.session;
        
        if (isNotConfirmed) {
          setMessage({ 
            text: "Account created! You can now try signing in.", 
            type: 'success' 
          });
        } else {
          setMessage({ 
        ...
        setMode('signin');
        setIdentifier(identifier.trim()); // Keep the username
      } else {
      ...
      setMessage({ text: err.message || 'A system error occurred.', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            {message && (
              <motion.p 
                ...
                className={cn(
                  "text-[11px] font-bold p-3 rounded-xl border",
                  message.type === 'error' 
